...
func TestConstructor(t *testing.T) {
	...
	t.Run("construct", func(t *testing.T) {
		rt := mock.NewBuilder(context.Background(), builtin.TokenActorAddr).
...
func TestToken(t *testing.T) {
	...
	t.Run("Create-MintBatch", func(t *testing.T) {
		...
		creatorsArray, err := adt.AsArray(rt.AdtStore(), st.Creators)
		assert.Nil(t, err)
		...
		balanceArray, err := adt.AsArray(rt.AdtStore(), st.Balances)
		assert.Nil(t, err)
		addrTokenAmountMap, found, err := st.LoadAddrTokenAmountMap(rt.AdtStore(), balanceArray, big.NewInt(1))
		assert.True(t, found)
		assert.Nil(t, err)
		ataMap, err := adt.AsMap(rt.AdtStore(), addrTokenAmountMap.AddrTokenAmountMap)
		assert.Nil(t, err)
		...
		isAllApproveMap, err := adt.AsMap(rt.AdtStore(), st.Approves)
		assert.Nil(t, err)
	...
	t.Run("URI", func(t *testing.T) {
		...
		urisArray, err := adt.AsArray(rt.AdtStore(), st.URIs)
		assert.Nil(t, err)
	...
	t.Run("Safe(Batch)TransferFrom", func(t *testing.T) {
		...
		assert.True(t, found)
		assert.Nil(t, err)
		ataMap1, err := adt.AsMap(rt.AdtStore(), addrTokenAmountMap1.AddrTokenAmountMap)
		assert.Nil(t, err)
		...
		assert.True(t, found)
		assert.Nil(t, err)
		ataMap2, err := adt.AsMap(rt.AdtStore(), addrTokenAmountMap2.AddrTokenAmountMap)
		assert.Nil(t, err)
	...
	t.Run("Approve-Transfer", func(t *testing.T) {
		...
		assert.True(t, found)
		assert.Nil(t, err)
		apMap, err := adt.AsMap(rt.AdtStore(), addrApproveMap.AddrApproveMap)
		assert.Nil(t, err)
